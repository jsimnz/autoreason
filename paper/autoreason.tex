...
\bottomrule
\end{tabular}
\caption{Iterative Borda scores (max~35) at 15~passes. Autoreason wins tasks 1, 3,~5; critique-and-revise wins tasks 2,~4. Compare Table~\ref{tab:v1}: iteration inverts the ranking.}
\label{tab:multi}
\end{table}

Single-pass autoreason averaged 22.2 Borda, placing 3rd on 4/5~tasks (Table~\ref{tab:v1}). With iteration, it won 3 of 5~tasks (avg 27.8, rank 1.4), outperforming all single-pass methods (Table~\ref{tab:multi}, Figure~\ref{fig:multi}). The judge's protection against bad revisions compounds: each pass either improves the incumbent or leaves it unchanged, while baselines accumulate drift unchecked.
...
Figure~\ref{fig:tree} shows the Task~1 trajectory. At pass~15, A had won twice consecutively. Eleven more passes followed---a 7-judge panel comparing pass~15 vs pass~25 chose pass~15 by 6--1. Post-convergence passes are noise.
...
Adding ``think step by step about each version'' to the judge prompt---no architecture changes---dramatically improves convergence (Table~\ref{tab:cot}). CoT judges produce scores of 8--9 for A wins (vs baseline's 6--7), acting as a debiasing mechanism: judges must articulate specific strengths before ranking, preventing verbosity bias.
...
\bottomrule
\end{tabular}
\caption{CoT reduces convergence 3$\times$ on Task~1. All subsequent experiments use CoT judges.}
\label{tab:cot}
\end{table}
...
Three evaluation-side modifications (anchored judges, subtractive synthesis, both combined) plus a scope-constrained task (500-word startup pitch from 8 fixed facts):
...
\bottomrule
\end{tabular}
\caption{Constrained task: autoreason wins with Sonnet~4.6. Compare Table~\ref{tab:scaling}: the baselines that won unconstrained now lose---they bloated past the 500-word limit.}
\label{tab:constrained_quality}
\end{table}

The ranking inverts (Figure~\ref{fig:scaling_story}). Critique-and-revise expanded to 932~words; autoreason stayed at 632. The constrained task penalizes exactly the drift that autoreason's architecture prevents.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig_scaling_story.pdf}
\caption{The scaling story. Same model (Sonnet~4.6), same method, opposite results. Left: unconstrained task, autoreason last (red). Right: scope-constrained task, autoreason first (blue).}
...
\textbf{Ground-truth critic.}\enspace Without ground-truth access, the initial Opus generation hallucinated a fabricated ablation study, fake confidence intervals, wrong model names, and incorrect role descriptions (Figure~\ref{fig:hallucination}). With ground-truth access, the critic caught all four on pass~1---mirroring peer review, where the reviewer checks supplementary materials.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{fig_hallucination.pdf}
\caption{4 hallucinated claims without ground truth; 0 with it.}
\label{fig:hallucination}
\end{figure}

\textbf{Judge panel integrity.}\enspace An earlier mixed panel (Opus + Sonnet + Gemini) failed to converge for 11+ passes because Gemini's output failed our parser, reducing the panel to 2 judges. Fixed to 3 working judges, the same incumbent converged in 2~passes (Figure~\ref{fig:integrity}). A broken judge doesn't add noise---it prevents equilibrium.
...
\textbf{CoT judges should be the default.}\enspace The cheapest improvement: 3$\times$ faster convergence, more decisive scores, no architecture changes.

\textbf{Convergence as stopping rule.}\enspace The threshold filters noise, not guarantees optimality. Post-convergence passes on Task~1 degraded quality. The Elo analysis confirms plateaus early regardless of whether the threshold is met.

\textbf{Scope is the central variable.}\enspace The bloat/prune oscillation with Sonnet~4 (Figure~\ref{fig:wordcount}) signals underdetermined scope. With stronger models, this oscillation becomes one-directional drift---but scope constraints eliminate it entirely, restoring both convergence and quality (Section~\ref{sec:scaling}). The synthesis operator needs a bounded improvement space. When the task constrains scope mechanically, the incumbent can reach a ceiling. When it doesn't, there is always something to add.
...
\textbf{Limitations.}\enspace All results from one model family (Anthropic). No human evaluation. The constrained-task result is from a single task---generalization untested. The paper-writing experiment used Opus, not Sonnet. Scaling remedy experiments each ran once; Monte Carlo replication would strengthen the claims.

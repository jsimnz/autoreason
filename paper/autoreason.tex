...
With Sonnet~4 across 5~subjective tasks, autoreason wins 3/5 outright. In competitive programming (81~CodeContests problems, matched compute budgets), it achieves 65\% private-test pass rate vs.\ 58\% for critique-and-revise, with the gap driven by structured recovery from failure ($p{=}0.041$). A judge ablation shows the tournament structure provides value even with weak judges, though strong judges amplify the effect. The practical recommendation: autoreason operates in the space between ``too weak to generate useful alternatives'' and ``too strong to need external evaluation''---the range where most cost-conscious LLM usage lives.

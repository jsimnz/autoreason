...
Self-refinement is unreliable in general. We identify the conditions under which it works and present autoreason, a method that exploits those conditions. Each iteration produces three competing versions (the incumbent, an adversarial revision, and a synthesis), evaluated by a blind judge panel using Borda count aggregation. With claude-sonnet-4 across 5~subjective tasks, autoreason ranked first or second in every 7-judge blind comparison, winning 3/5~outright. With the stronger Sonnet~4.6, the method fails on open-ended tasks but wins on scope-constrained tasks, and eight remedy experiments identify scope as the root cause.

We validate these findings in competitive programming (81~CodeContests problems across 3~difficulty tiers). With matched compute budgets (6~LLM calls per problem), autoreason achieves 65\% private-test pass rate vs.\ 58\% for both single-pass sampling and standard critique-and-revise. Crucially, critique-and-revise overfits to visible test cases: it matches autoreason on public tests (79\% vs.\ 85\%) but fails to generalize to hidden tests. Autoreason's structured reasoning protects against this overfitting, with the strongest gains on hard problems (47\% vs.\ 37\% private pass rate). The method works when three conditions hold: constrained scope, external verification signal, and structured reasoning that goes beyond pattern-matching on feedback.
...
Our central finding is that self-refinement works reliably when three conditions hold: (1)~\textbf{constrained scope}---the task has bounded improvement space; (2)~\textbf{external verification}---feedback comes from outside the generating model's reasoning loop; and (3)~\textbf{structured reasoning}---the refinement process explicitly analyzes failure modes rather than pattern-matching on feedback. We establish these conditions across two domains: subjective writing (5~tasks, 8~remedy experiments) and competitive programming (81~CodeContests problems). In the code domain, autoreason's structured reasoning prevents the overfitting to visible test cases that standard critique-and-revise exhibits, yielding a 7-point improvement in hidden-test generalization.
...
The connection to PSRO~\cite{psro} is loosely analogous rather than a direct equivalence: each pass adds a candidate to an empirical game, but autoreason lacks PSRO's full meta-strategy computation. Our retroactive analysis (Section~\ref{sec:retro}) examines this framing empirically: preferences are near-transitive across 91~passes (1.1\% Condorcet cycle rate), and Elo ratings plateau early, consistent with the Markov chain reaching its absorbing state.
...
\textbf{Autoresearch}~\cite{autoresearch} uses val\_bpb as an objective fitness function; autoreason extends this beyond objective metrics. \textbf{SlopCodeBench}~\cite{slopcodebench} shows iterative code degradation that prompting can't fix. \textbf{ACE}~\cite{ace} documents context collapse in iterative rewriting. \textbf{Self-Refine}~\cite{selfrefine} demonstrates single-agent iterative improvement but Huang et al.~\cite{huang2023} show self-correction is unreliable without external feedback; our code experiment (Section~\ref{sec:code}) quantifies this: critique-and-revise overfits to visible test cases while structured reasoning generalizes. \textbf{AI Safety via Debate}~\cite{irving2018} grounds adversarial evaluation: truth has a natural advantage when agents argue before a judge. \textbf{Constitutional AI}~\cite{bai2022} uses principle-based self-evaluation as training signal. \textbf{LLM-as-Judge}~\cite{zheng2023} documents systematic biases motivating our randomization and panel design. \textbf{LLM Council}~\cite{llmcouncil} uses multi-model panels for single-turn evaluation; autoreason adds iteration with role separation. \textbf{PSRO}~\cite{psro} provides game-theoretic framing for iterative strategy generation, loosely analogous to our pass structure. \textbf{Verdict}~\cite{verdict} scales judge-time compute through hierarchical verification and debate-aggregation; we tested its rank-then-verify pattern and found it does not address the scope problem (Section~\ref{sec:scaling}). \textbf{CodeContests}~\cite{codecontests} provides competitive programming problems with hidden test cases; we use it to validate autoreason in an objectively verifiable domain.

\section{Discussion}

\textbf{Three conditions for reliable self-refinement.}\enspace Across both domains, autoreason works when: (1)~scope is constrained (500-word pitch, competitive programming problem), (2)~external verification exists (judge panel, test suite), and (3)~reasoning is structured rather than reactive. Remove any one and the method degrades: unconstrained scope causes drift (Section~\ref{sec:scaling}), self-evaluation without external signal is unreliable~\cite{huang2023}, and shallow critique-and-revise overfits to visible feedback (Section~\ref{sec:code}).
...
\textbf{Limitations.}\enspace All results come from one model family (Anthropic). Writing evaluation uses LLM judges, not human raters; the quality claims rest on LLM preferences. The code experiment provides objective verification but the overall private-test pass rate difference (64\% vs.\ 58\%) is not statistically significant at $p{<}0.05$; only the paired recovery analysis reaches significance. The constrained writing task is a single task (500-word pitch); the code domain provides broader validation (150~problems) but both use the same model. Scaling remedy experiments each ran once. Monte Carlo $n{=}5$ is underpowered.

\section{Conclusion}

Self-refinement is unreliable in general, but it works under identifiable conditions. We find three: constrained scope, external verification, and structured reasoning. Autoreason exploits all three. In subjective writing, it ranked first or second across 5~tasks with Sonnet~4 and first on a scope-constrained task with the stronger Sonnet~4.6. In competitive programming, it recovers significantly more failed solutions than alternatives ($p{=}0.041$), with the strongest gains when the model must reason about algorithmic failures rather than pattern-match on error messages.

The practical recommendation: pair iterative refinement with explicit scope constraints and external feedback signals, and invest the refinement budget in structured analysis of failures rather than additional generation attempts. The broader insight: the obstacle to iterative LLM self-improvement is not model capability but the absence of grounding. Without a signal external to the model's own reasoning loop, refinement is an echo chamber. With one, it converges.

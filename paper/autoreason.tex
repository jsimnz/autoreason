...
Iterative self-refinement fails because LLMs cannot reliably evaluate their own outputs. We present autoreason, a method that replaces self-evaluation with external evaluation: each iteration produces three competing versions (the incumbent, an adversarial revision, and a synthesis), judged by a blind panel using Borda count. The method's value scales inversely with model capability. With a cheap model (Haiku~3.5, ${\sim}$10$\times$ cheaper than Sonnet~4), autoreason scored 42/42 Borda across three tasks---a perfect sweep of 21/21 first-place votes---while standard refinement baselines \textit{degraded} the same model's outputs below its unrefined single pass. The advantage margin is 2.6$\times$ larger for Haiku than for Sonnet~4, because weaker models have a wider gap between generation and self-evaluation capability.

With Sonnet~4 across 5~subjective tasks, autoreason wins 3/5 outright. In competitive programming (81~CodeContests problems, matched compute budgets), it achieves 65\% private-test pass rate vs.\ 58\% for critique-and-revise, with the gap driven by structured recovery from failure ($p{=}0.041$). With the stronger Sonnet~4.6, the method fails on open-ended tasks but wins on scope-constrained tasks (2/3), and eight remedy experiments identify scope as the root variable. The practical recommendation: spend your refinement budget on your weakest models---that is where external evaluation structure has the highest ROI.
...
Our central finding is that \textbf{the value of structured refinement scales inversely with model capability}. Self-refinement fails because it requires a model to evaluate its own outputs---a task that is harder than generation itself. The weaker the model, the wider this gap between what it can produce and what it can assess, and the more it benefits from external evaluation. With Haiku~3.5 (${\sim}$10$\times$ cheaper than Sonnet~4), autoreason achieved a perfect 42/42 Borda score across three tasks while standard refinement baselines \textit{degraded} the model's outputs below its unrefined single pass (Section~\ref{sec:small}). The advantage margin is 2.6$\times$ larger for Haiku than for Sonnet~4. With the even stronger Sonnet~4.6, the method's advantage narrows further and becomes scope-dependent (Section~\ref{sec:scaling}).

We establish four conditions under which the method works: (1)~\textbf{external verification}---the critical factor, providing evaluation capability the generating model lacks; (2)~\textbf{constrained scope}---bounded improvement space; (3)~\textbf{structured reasoning}---explicit failure analysis rather than reactive editing; and (4)~\textbf{sufficient decision space}---the task admits genuinely different valid approaches. We validate across subjective writing (5~open-ended tasks, 3~constrained tasks, 8~remedy experiments), competitive programming (81~CodeContests problems), and cross-model capability comparison (Haiku~3.5 vs.\ Sonnet~4 vs.\ Sonnet~4.6).
...
The experiments above use a single frontier model for all roles. A practical question: does autoreason help \textit{weaker} models more? We ran all five refinement methods with Claude Haiku~3.5 (${\sim}$10$\times$ cheaper than Sonnet~4) as the author/critic/synthesizer, keeping Sonnet~4 judges for evaluation. Three tasks: the constrained pitch, constrained policy, and open-ended GTM strategy.
...
Autoreason achieved a perfect score across all three tasks and all 21~judges. More striking: every refinement baseline scored \textit{below} the unrefined single pass on average (Table~\ref{tab:small_model}). Critique-and-revise averaged 16.3 vs.\ single-pass's 33.7---iterative self-refinement actively harms weaker models. The outputs confirm this qualitatively: after 15~passes, Haiku's baselines shrank to 78--192 words (from 205--345 initial), losing content with each pass.

\textbf{Disproportionate advantage.}\enspace With Sonnet~4, autoreason's margin over the best baseline was +5.4 Borda (27.8 vs.\ 22.4, normalized: +7.7pp). With Haiku~3.5, the margin is +8.3 (42.0 vs.\ 33.7, normalized: +19.8pp)---a 2.6$\times$ larger relative advantage. The weaker the model, the more it benefits from external evaluation structure, because weaker models cannot reliably distinguish improvement from degradation in their own outputs. Autoreason's judge panel provides exactly this missing capability.
...
\textbf{The generation-evaluation gap.}\enspace Self-refinement requires a model to both generate and evaluate. These are different tasks, and evaluation is often harder~\cite{huang2023}. The stronger the model, the narrower this gap: Sonnet~4.6 can evaluate its own outputs well enough that external judges add little value on unconstrained tasks. Haiku~3.5 cannot---its refinement baselines actively degrade outputs (Table~\ref{tab:small_model}) because it cannot distinguish improvement from damage. Autoreason bridges this gap by outsourcing evaluation to a stronger model, and the value of doing so scales with the size of the gap. This explains why the advantage margin is 2.6$\times$ larger for Haiku than for Sonnet~4, and why it narrows further with Sonnet~4.6.
...
Self-refinement fails not because models lack capability, but because they lack the ability to evaluate their own outputs. The weaker the model, the wider this generation-evaluation gap, and the more valuable external evaluation becomes. Autoreason bridges the gap: with Haiku~3.5, it achieved a perfect score across all tasks while standard baselines degraded outputs below the unrefined starting point. With Sonnet~4, the advantage narrows to a modest lead. With Sonnet~4.6, it becomes scope-dependent.

The practical recommendation: spend your refinement budget on your weakest models. A cheap model with autoreason structure and strong judges outperforms the same cheap model with any self-refinement strategy by a wide margin. For frontier models, the ROI is lower---use scope constraints and structured reasoning, but don't expect the same gains. The broader insight: the obstacle to iterative LLM self-improvement is not model capability but the absence of a signal external to the model's own reasoning loop. Without one, refinement is an echo chamber. With one, it converges---and the value of convergence is greatest for models that need it most.

...
\begin{center}
\vspace{4pt}
{\LARGE\bfseries Autoreason: Iterative Refinement Scales Inversely\\[3pt] with Model Capability}\\[10pt]
...
Iterative self-refinement fails because LLMs cannot reliably evaluate their own outputs. We present autoreason, a method that replaces self-evaluation with external evaluation: each iteration produces three competing versions (the incumbent, an adversarial revision, and a synthesis), judged by a blind panel using Borda count. The method's value peaks at mid-tier models, where the gap between generation capability and self-evaluation capability is widest. With Haiku~3.5 (${\sim}$10$\times$ cheaper than Sonnet~4), autoreason scored 42/42 Borda across three tasks---a perfect sweep---while standard refinement baselines \textit{degraded} the same model's outputs below its unrefined single pass. Across four model tiers (Llama~8B, Gemini~Flash, Haiku~3.5, Sonnet~4), the advantage peaks at mid-tier and diminishes at both extremes: models too weak to generate diverse alternatives (Llama) or too strong to need external evaluation (Sonnet~4.6).

With Sonnet~4 across 5~subjective tasks, autoreason wins 3/5 outright. In competitive programming, it achieves 65\% private-test pass rate vs.\ 58\% for critique-and-revise ($p{=}0.041$). On CodeContests with Haiku authors, autoreason matches Sonnet~4 single-pass on hard problems (4/10 each) at ${\sim}$10$\times$ lower author cost, while critique-and-revise degrades Haiku below its unrefined baseline---objectively confirming ``refinement as destruction'' with test cases, not judge preferences. Autoreason also outperforms best-of-6 sampling with judge selection on every task, demonstrating that structured refinement adds value beyond diversity. The practical recommendation: autoreason operates in the space between ``too weak to generate useful alternatives'' and ``too strong to need external evaluation''---the range where most cost-conscious LLM usage lives.
...
Our central finding is that \textbf{the value of structured refinement depends on the gap between a model's generation capability and its self-evaluation capability}. Self-refinement fails because evaluation is harder than generation~\cite{huang2023}. Autoreason bridges this gap with external evaluation. Across four model tiers, the advantage peaks at mid-tier models where the gap is widest: Haiku~3.5 achieved a perfect 42/42 Borda while standard baselines \textit{degraded} its outputs below the unrefined starting point (Section~\ref{sec:small}). At the bottom (Llama~8B), the model is too weak to generate diverse alternatives for the tournament. At the top (Sonnet~4.6), the gap is narrow enough that simple methods suffice (Section~\ref{sec:scaling}). The method operates in the practical middle---the range where most cost-conscious LLM deployment lives, and where the generation-evaluation gap is large enough to exploit.

We establish four conditions under which the method works: (1)~\textbf{external verification}---the critical factor, providing evaluation capability the generating model lacks; (2)~\textbf{constrained scope}---bounded improvement space; (3)~\textbf{structured reasoning}---explicit failure analysis rather than reactive editing; and (4)~\textbf{sufficient decision space}---the task admits genuinely different valid approaches. We validate across subjective writing (5~open-ended tasks, 3~constrained tasks, 8~remedy experiments), competitive programming (81~CodeContests problems), a four-tier model capability comparison (Llama~8B through Sonnet~4), and a judge ablation that isolates the evaluation mechanism.
...
\bottomrule
\end{tabular}
\caption{Haiku~3.5 with all refinement methods (Borda /42, 7 Sonnet~4 judges). Autoreason scored 42/42 on every task---21/21 first-place votes. Refinement baselines \textit{degrade} Haiku outputs below the unrefined single pass.}
...
A natural concern: does autoreason add value beyond sampling multiple outputs and selecting the best? We compared autoreason against best-of-6 (6 independent outputs, Sonnet judges select the winner---matched compute budget).
...
To validate the capability-scaling finding with objective ground truth, we ran autoreason with Haiku authors on 30~CodeContests problems (10~easy, 10~medium, 10~hard), using test cases rather than LLM judges.
...
\midrule
\rowcolor{TableShade}
Sonnet single & \textbf{83\%} & \textbf{63\%} & 8 & 7 & 4 \\
\textbf{Haiku AR} & 67\% & 53\% & 6 & 6 & \textbf{4} \\
\rowcolor{TableShade}
Haiku single & 60\% & 50\% & 5 & 5 & 5 \\
Haiku C\&R & 47\% & 37\% & 4 & 4 & 3 \\
\bottomrule
\end{tabular}
\caption{CodeContests private-test pass rate (30 problems, 6 LLM calls each). Haiku autoreason closes half the gap to Sonnet (50\%$\to$53\% vs.\ 63\%) and \textbf{matches Sonnet on hard problems} (4/10 each). Critique-and-revise degrades Haiku below single pass (37\% vs.\ 50\%).}
\label{tab:haiku_code}
\end{table}

Three findings confirm the writing experiments with objective evidence: (1)~critique-and-revise harms Haiku (37\% vs.\ 50\% single-pass), replicating ``refinement as destruction''; (2)~autoreason improves Haiku (53\% vs.\ 50\%), with gains driven by structured failure analysis; (3)~on hard problems, Haiku autoreason matches Sonnet single-pass (4/10 each) at ${\\sim}$10$\\times$ lower author cost. The generation-evaluation gap is real and measurable with test cases, not just LLM judge preferences.
...
Self-refinement fails not because models lack capability, but because they lack the ability to evaluate their own outputs. Autoreason bridges this generation-evaluation gap with a tournament structure and external judges. The method's value is not monotonically inverse to capability---it peaks at mid-tier models where the gap is widest, and diminishes at both extremes. Llama~8B cannot generate diverse enough alternatives for the tournament to select between. Sonnet~4.6 can already self-evaluate well enough that external judges add little. The sweet spot---Haiku~3.5, Gemini~Flash, and to a lesser extent Sonnet~4---is where the model can produce meaningfully different candidates but cannot reliably choose between them.

This sweet spot is not a fixed point. As model capabilities improve and costs decrease, today's frontier becomes tomorrow's mid-tier. The generation-evaluation gap is a structural property of LLMs at any capability level, not an artifact of current models. The practical recommendation: match your refinement architecture to the model's position on the capability curve. For mid-tier models, autoreason provides gains that no self-refinement baseline can match. For frontier models, use scope constraints and structured reasoning. For the weakest models, invest in generation quality first.

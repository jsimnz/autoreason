...
Judges maintain better calibration with baselines. Our baseline visibility test showed unanimous preference (7-0) with baseline against an adversarial version, demonstrating that judges need reference points to detect drift effectively.
...
Autoreason demonstrates one approach to iterative refinement in subjective domains. By isolating agents, implementing blind evaluation, and using conservative aggregation, the method achieved improvements in our test case while limiting the length inflation seen in baseline approaches.
